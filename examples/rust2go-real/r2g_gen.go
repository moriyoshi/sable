...
typedef struct OrderRef {
  struct StringRef item;
  uint8_t qty;
} OrderRef;
const void c_DemoCall_demo_check_spawn(const void*);
*/
import "C"
import (
	"runtime"
...
func newOrder(p C.OrderRef) Order {
	return Order{
		item: newString(p.item),
		qty:  newC_uint8_t(p.qty),
	}
}
func ownOrder(p C.OrderRef) Order {
	return Order{
		item: ownString(p.item),
		qty:  newC_uint8_t(p.qty),
	}
}
...
func refOrder(p *Order, buffer *[]byte) C.OrderRef {
	return C.OrderRef{
		item: refString(&p.item, buffer),
		qty:  refC_uint8_t(&p.qty, buffer),
	}
}

type DemoCallImpl struct{}

func (DemoCallImpl) demo_check_spawn(order *Order, rt *uint64, token *uint64) {
	_internal_params := [3]unsafe.Pointer{}
	order_ref, order_buffer := cvt_ref(cntOrder, refOrder)(order)
	_internal_params[0] = unsafe.Pointer(&order_ref)
	rt_ref, rt_buffer := cvt_ref(cntC_uint64_t, refC_uint64_t)(rt)
	_internal_params[1] = unsafe.Pointer(&rt_ref)
	token_ref, token_buffer := cvt_ref(cntC_uint64_t, refC_uint64_t)(token)
	_internal_params[2] = unsafe.Pointer(&token_ref)
	cgocall.CallFuncG0P1(unsafe.Pointer(C.c_DemoCall_demo_check_spawn), unsafe.Pointer(&_internal_params))
	runtime.KeepAlive(_internal_params)
	runtime.KeepAlive(order_buffer)
	runtime.KeepAlive(rt_buffer)
	runtime.KeepAlive(token_buffer)
}